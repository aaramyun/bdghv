...
\newcommand*{\CDGHV}{\ensuremath{\mathsf{GDGHV}}}
...
\item[$\CDGHV.\KeyGen(1^\lambda,\mu,\ell)$.] Choose $\eta$-bit distinct primes $p_j$, $0\leq j<\ell$, and denote $\pi$
...
$x_0=q_0\cdot\pi$, where $q_0 \leftarrow \Z \cap [0,2^{\gamma}/\pi)$
  ...
  Choose $\mu$-bit integers $Q_j$ with $\gcd(Q_j,x_0)=1$ for $0\leq j<\ell$. 
...
\leftarrow\Z\cap(-2^{\rho}, 2^{\rho}) \\[.2cm]
...
\end{equation}
\item[$\CDGHV.\Decrypt(\sk, c)$.] Output $\vec{m}=(m_0,\ldots,m_{\ell-1})$ where $m_j \leftarrow [c]_{p_j} \bmod Q_j$.
\\\vspace{-0.25cm}
...
The parameter $\mu$ can be an integer from $2$ to $\eta/8$ depending on the multiplicative depth of the scheme. The details of analysis are given in Appendix~\ref{app:correctness}.
...
\rho=2\lambda$, $\tau=\gamma+\lambda$ which is similar to the DGHV's convenient parameter setting \cite{vDGHV2010}. 
...
The $(\rho,\eta,\gamma)$-decisional approximate GCD problem is: For a random $\eta$-bit prime $p$, given a $\gamma$-bit integer  $x_0=q_0\cdot p$ and polynomially many samples from $\mathcal D_{\rho}(p,q_0)$,
%and a set $S:=\{x_i \mod x_0 \big|$ for polynomially many samples $x_i\leftarrow \D_{\gamma,\rho}(p)\}$, 
determine $b\in\{0,1\}$ from $z=x+r\cdot b\mod x_0$ where $x\leftarrow\mathcal D_{\rho}(p,q_0)$ and $r\leftarrow\Z\cap[0,x_0)$.
\end{definition}

We prove the following Lemma in Appendix~\ref{p:red sec}.
\begin{lemma}
\label{le:redsec}
