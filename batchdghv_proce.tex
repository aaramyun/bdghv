...
\item Brakerski and Vaikuntanathan's scheme based on the Learning with
  ...
  problems~\cite{BV2011a,BV2011b}, and follow-up works (\eg the scale-free variant of Brakerski~\cite{Bra2012} and the NTRU-variant~\cite{LTV2012}). An implementation is described in
  \cite{GHS2012c} with an efficient (given the current state of knowledge)
  homomorphic evaluation of the full AES encryption circuit.  The
  authors use the \emph{batch} RLWE scheme proposed in \cite{BGV2012,GHS2012a},
  that allows one to
 ...
 manipulating only the ciphertext. 
\end{enumerate}

\subsubsection{Our Contributions.} 

In this paper we focus on the DGHV scheme. Our goal is to extend DGHV to
support the same batching capability as in  RLWE based
...
a slight increase of the public key size). 
...
significantly improved efficiency over the RLWE scheme of
\cite{GHS2012a}, we believe it is interesting to obtain FHE schemes with
...
\begin{definition}[EF-$\ell$-dAGCD$_{\lambda,\gamma,\eta}$] The
...
bootstrappable. In the original DGHV scheme, the decryption equation
was:
\begin{equation}
\label{eq:msz}
...
From Gentry's theorem, we obtain homomorphic encryption schemes for
...
values between plaintext slots with a $\Permute$ operation. As opposed to
\cite{GHS2012a} we cannot rely on an underlying algebraic structure.
...
As mentioned 
in~\cite{GHS2012c,NLV2011}, such an implementation can be used to
...
\begin{figure}[ht]
  ...
  \draw (0,1) rectangle (0.25,2);
  \node[rotate=90,scale=0.8] at (0.125, 1.5) {\tiny AES 1};
  \draw (0.25,1) rectangle (0.5,2);
  \node[rotate=90,scale=0.8] at (0.375, 1.5) {\tiny AES 2};
  \draw (0.5,1) rectangle (0.75,2);
  \node[rotate=90,scale=0.8] at (0.625, 1.5) {\tiny AES 3};
  \draw (0.75,1) rectangle (1.75,2);
  \node at (1.25, 1.5) {\ldots};
  \draw (1.75,1) rectangle (2,2);
  \node[rotate=90,scale=0.8] at (1.875, 1.5) {\tiny AES $k$};
  
  \draw (2,1) rectangle (2.25,2);
  \node[rotate=90,scale=0.8] at (2.125, 1.5) {\tiny AES 1};
  \draw (2.25,1) rectangle (2.5,2);
  \node[rotate=90,scale=0.8] at (2.375, 1.5) {\tiny AES 2};
  \draw (2.5,1) rectangle (2.75,2);
  \node[rotate=90,scale=0.8] at (2.625, 1.5) {\tiny AES 3};
  \draw (2.75,1) rectangle (3.75,2);
  \node at (3.25, 1.5) {\ldots};
  \draw (3.75,1) rectangle (4,2);
  \node[rotate=90,scale=0.8] at (3.875, 1.5) {\tiny AES $k$};
  
  \draw (4,1) rectangle (4.25,2);
  \node[rotate=90,scale=0.8] at (4.125, 1.5) {\tiny AES 1};
  \draw (4.25,1) rectangle (4.5,2);
  \node[rotate=90,scale=0.8] at (4.375, 1.5) {\tiny AES 2};
  \draw (4.5,1) rectangle (4.75,2);
  \node[rotate=90,scale=0.8] at (4.625, 1.5) {\tiny AES 3};
  \draw (4.75,1) rectangle (5.75,2);
  \node at (5.25, 1.5) {\ldots};
  \draw (5.75,1) rectangle (6,2);
  \node[rotate=90,scale=0.8] at (5.875, 1.5) {\tiny AES $k$};
  
  \draw (6,1) rectangle (6.25,2);
  \node[rotate=90,scale=0.8] at (6.125, 1.5) {\tiny AES 1};
  \draw (6.25,1) rectangle (6.5,2);
  \node[rotate=90,scale=0.8] at (6.375, 1.5) {\tiny AES 2};
  \draw (6.5,1) rectangle (6.75,2);
  \node[rotate=90,scale=0.8] at (6.625, 1.5) {\tiny AES 3};
  \draw (6.75,1) rectangle (7.75,2);
  \node at (7.25, 1.5) {\ldots};
  \draw (7.75,1) rectangle (8,2);
  \node[rotate=90,scale=0.8] at (7.875, 1.5) {\tiny AES $k$};
  
  \node at (9, 1.5) {\ldots};
  
  \draw (10,1) rectangle (10.25,2);
  \node[rotate=90,scale=0.8] at (10.125, 1.5) {\tiny AES 1};
  \draw (10.25,1) rectangle (10.5,2);
  \node[rotate=90,scale=0.8] at (10.375, 1.5) {\tiny AES 2};
  \draw (10.5,1) rectangle (10.75,2);
  \node[rotate=90,scale=0.8] at (10.625, 1.5) {\tiny AES 3};
  \draw (10.75,1) rectangle (11.75,2);
  \node at (11.25, 1.5) {\ldots};
  \draw (11.75,1) rectangle (12,2);
  \node[rotate=90,scale=0.8] at (11.875, 1.5) {\tiny AES $k$};
  
  \node at (13, 1.5) {\ldots};
  
  \draw (14,1) rectangle (14.25,2);
  \node[rotate=90,scale=0.8] at (14.125, 1.5) {\tiny AES 1};
  \draw (14.25,1) rectangle (14.5,2);
  \node[rotate=90,scale=0.8] at (14.375, 1.5) {\tiny AES 2};
  \draw (14.5,1) rectangle (14.75,2);
  \node[rotate=90,scale=0.8] at (14.625, 1.5) {\tiny AES 3};
  \draw (14.75,1) rectangle (15.75,2);
  \node at (15.25, 1.5) {\ldots};
  \draw (15.75,1) rectangle (16,2);
  \node[rotate=90,scale=0.8] at (15.875, 1.5) {\tiny AES $k$};
  
  % Middle
  \draw (0,2) rectangle (2,2.5);
  \node at (1, 2.25) {Row 0};
  \draw (0,2) rectangle (4,2.5);
  \node at (3, 2.25) {Row 1};
  \draw (0,2) rectangle (6,2.5);
  \node at (5, 2.25) {Row 2};
  \draw (0,2) rectangle (8,2.5);
  \node at (7, 2.25) {Row 3};
  \draw (0,2) rectangle (10,2.5);
  \node at (9, 2.25) {\ldots};
  \draw (0,2) rectangle (12,2.5);
  \node at (11, 2.25) {Row 0};
  \draw (0,2) rectangle (14,2.5);
  \node at (13, 2.25) {\ldots};
  \draw (0,2) rectangle (16,2.5);
  \node at (15, 2.25) {Row 3};
  
  
  % Top
  \draw (0,2.5) rectangle (8,3);
  \node at (4, 2.75) {Column 0};
  \draw (8,2.5) rectangle (10,3);
  \node at (9, 2.75) {\ldots};
  \draw (10,2.5) rectangle (16,3);
  \node at (13, 2.75) {Column 3};
  \end{tikzpicture}
...
Boyar and Peralta~\cite{BP2010} to compute the Sbox. 
It is shown in~\cite{LP2013} that this circuits only needs $17$
$\Recrypt$ operations
on $9$ of
...
Our timings are comparable to~\cite{GHS2012c} for the RLWE
based scheme, where a relative time of $5$ minutes per block is
